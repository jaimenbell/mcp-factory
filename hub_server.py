...
Factory (default):  generate a server stub + claude.json entry from one manifest
  python hub_server.py --manifest <path> [--output-dir DIR] [--force] [--dry-run] [--verify PATH]
...
Serve:  run as a live MCP hub server (stdio transport, registers with Claude)
  python hub_server.py --serve [--scan-root ROOT [--scan-root ROOT2 ...]]

Combining:
  python hub_server.py --scan --apply && python hub_server.py --serve
"""
...
_DEFAULT_OUTPUT_DIR = Path(__file__).parent / "generated"
